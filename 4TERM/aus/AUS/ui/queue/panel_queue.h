...
namespace ui
{
	...
	/// </summary>
	public ref class PanelPresentQueue : public UserControl
	{
	public:
		PanelPresentQueue(void);

		void initialize(ControlsHolder^ holder);
		void visualize(structures::Queue<data::DataType>* queue);
	protected:
		~PanelPresentQueue();
	private:
...
#pragma endregion
	...
	PanelPresentQueue::PanelPresentQueue(void) :
		controls(nullptr)
	{
		InitializeComponent();
	}

	inline void PanelPresentQueue::initialize(ControlsHolder ^ holder)
	{
		controls = holder;
	}

	PanelPresentQueue::~PanelPresentQueue()
	{
		if (components)
		{
			delete components;
	...
	void PanelPresentQueue::visualize(structures::Queue<data::DataType>* queue)
	{
		size_t size = queue->size();

		if (size > 0)
		{
			data::DataType data = queue->peek();
